...
const Home = () => {
  ...
  const fetchNFTs = async() => {
    ...
    const api_key = "UgyXWavn7p_XSdmx-tCa-DqEe5iucf7d";
    ...
    var requestedOptions = {
      ...
      //wallet address being a url parameter
      nfts = await fetch(fetchURL, requestedOptions).then(data => data.json())
    } else {
      console.log("fetching nfts for collection owned by address");
      const fetchURL = `${base_url}?owner=${wallet}&contractAddresses%5B%D=${collection}`;
      //wallet address and collection address being url parameters
      //& is a separator between two paramters 
      nfts = await fetch(fetchURL, requestedOptions).then(data => data.json());
